...
# check root
[[ $EUID -ne 0 ]] && LOGE "mistake:  This script must be run as root user!\n" && exit 1
...
confirm_restart() {
    confirm "Whether to restart the panel，Rebooting the panel will also reboot xray" "y"
    if [[ $? == 0 ]]; then
        restart
    else
        show_menu
...
update() {
    confirm "This feature will force the latest version to be reinstalled，Data will not be lost，Whether to continue?" "n"
    if [[ $? != 0 ]]; then
        LOGE "Cancelled"
        if [[ $# == 0 ]]; then
            before_show_menu
...
uninstall() {
    ...
    echo ""
    echo -e "Uninstalled successfully，If you want to remove this script，run after exiting the script ${green}rm /usr/bin/x-ui -f${plain} to delete"
    echo ""
...
ssl_cert_issue() {
    ...
    if [ $? -eq 0 ]; then
        ...
        if [ $? -ne 0 ]; then
            ...
            LOGI "The certificate is issued successfully, the installation is in progress..."
        fi
...
show_usage() {
    ...
    echo "x-ui status       - Check x-ui 状态"
    echo "x-ui enable       - set up x-ui 开机自启"
    echo "x-ui disable      - Cancel x-ui 开机自启"
    echo "x-ui log          - Check x-ui 日志"
    echo "x-ui v2-ui        - Migrate the v2-ui account data of this machine to x-ui"
    echo "x-ui update       - renew x-ui panel"
